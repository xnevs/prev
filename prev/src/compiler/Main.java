...
import compiler.proepi.*;
import compiler.mmsfilewriter.*;
...
public class Main {
	...
	private static String execPhase = "all";
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			FrmEvaluator frmEvaluator = new FrmEvaluator();
			ImcCodeGen imcodegen = new ImcCodeGen();
			
			// Standardna knjiznica
			AbsTree standardLibrary = StandardLibrary.standardDefs();
			standardLibrary.accept(nameChecker);
			standardLibrary.accept(typeChecker);
			standardLibrary.accept(frmEvaluator);
			
			// Semanticna analiza.
			SemAn semAn = new SemAn(dumpPhases.contains("seman"));
			source.accept(nameChecker);
			source.accept(typeChecker);
			semAn.dump(source);
			if (execPhase.equals("seman")) break;
			// Klicni zapisi.
			Frames frames = new Frames(dumpPhases.contains("frames"));
			source.accept(frmEvaluator);
			frames.dump(source);
			...
			// Analiza zacasnih spremenljivk
			TmpAn tmpan = new TmpAn(dumpPhases.contains("tmpan"));
			tmpan.analyse(imcodegen.chunks);
			...
			ProEpi proEpi = new ProEpi();
			proEpi.generate(imcodegen.chunks);
			
			MMSFileWriter mmsFileWriter = new MMSFileWriter(sourceFileName.replaceFirst("\\.prev$", "") + ".mms");
			mmsFileWriter.write(imcodegen.chunks);
			
			if (execPhase.equals("all")) break;
